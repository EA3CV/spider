...
BEGIN {
    ...
    # Añadir rutas locales a @INC
    unshift @INC, "$root/perl5lib" unless grep {$_ eq "$root/perl5lib"} @INC;
    unshift @INC, "$root/perl" unless grep {$_ eq "$root/perl"} @INC;
    unshift @INC, "$root/local" unless grep {$_ eq "$root/local"} @INC;

    # Validar existencia de directorios y archivos críticos
    ...
    # Crear y bloquear archivo de lock
    $lockfn = "$root/local_data/cluster.lck";
    if (-w $lockfn) {
    ...
    $is_win = ($^O =~ /^MS/ || $^O =~ /^OS-2/) ? 1 : 0;
    $systime = time;

    # Cargar local::lib si existe
    eval {
        require local::lib;
    };
    unless ($@) {
    ...
    # Cargar DXVars para obtener $dsn, $dbuser, etc.
    require DXVars;

    # Cargar utilidades SQL y verificar/crear base de datos y usuario
    require SQL_Utils;
    SQL_Utils::ensure_database_ready();
}
...
#warn "[DEBUG cluster.pl] \$main::mycall = '$main::mycall'";
#my $ref = DXUser::get($main::mycall);
#warn "[DEBUG cluster.pl] get() devuelve: " . (defined $ref ? "OK" : "undef");
